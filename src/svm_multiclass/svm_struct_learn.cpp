							...
							STRUCTMODEL *sm, int alg_type)
{
	...
	*/
	...
	} while( numIt < lparm->maxiter && (cached_constraint || (ceps > sparm->epsilon) || 
...
void svm_learn_struct_joint(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
							LEARN_PARM *lparm, KERNEL_PARM *kparm, 
							STRUCTMODEL *sm, int alg_type, double *initW, const int &nParam )
{
	int         i,j;
	int         numIt=0;
	long        argmax_count=0;
	...
	*/
	...
	sm->svm_model=svmModel;
	sm->w=svmModel->lin_weights; /* short cut to weight vector */
	for ( int i=1; i<=nParam; i++ ) { // set the initial value for warm start.
		sm->w[i] = initW[i];
	}
